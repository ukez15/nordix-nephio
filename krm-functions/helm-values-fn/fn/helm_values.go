...
type HelmValues struct {
	HelmValuesMetadata metav1.ObjectMeta
	Params             map[string]string
	Data               []HelmValuesEntry
}

func (entry *HelmValuesEntry) generate(params, data map[string]string) error {
	switch entry.Type {
	case "gotmpl":
...
func (fc *HelmValues) Validate() error {
...
func Process(rl *fn.ResourceList) (bool, error) {
	...
	name := fc.HelmValuesMetadata.Name
	if name == "" {
	...
	err = cmko.SetKind("HelmValues")
	if err != nil {
		return false, err
	}
	err = cmko.SetNestedField(fc.HelmValuesMetadata, "metadata")
	if err != nil {
		return false, err
	}
	...
	err = cmko.SetAnnotation(kioutil.PathAnnotation, "_helm_values_"+name+".yaml")
	if err != nil {
		return false, err
	}
